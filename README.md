...
As we are here, let us discuss few concepts in load balancer.

### Sticky Sessions
Sticky sessions (also known as session affinity) in an Application Load Balancer (ALB) allow the load balancer to bind a user's session to a specific target (EC2 instance) within the target group. This ensures that all requests from the user during the session are sent to the same target, maintaining session continuity.

This sticky sessions are useful when we are configuring stateful application behind a load balancer (mostly legacy implemetations). Stateful applications are applications which store information on the system between the requests (user cart, profiles). ALB uses cookies in the requests to identify the same target everytime and routes the traffic to it.

Problem with sticky session is when we are doing this, one instance may get overloaded. Which affects scalability. inconsistency (if the instance is replaced) and response time.

### Ways to deal
1. One way to overcome the scalability issue in these legacy system is moving the session data to a centralized storage  or in memory cache (Redis).
2. Optimize the load balancing algorithm to distribute traffic more evenly across instances. For example, use "least connections" or "least outstanding requests" rather than simple round-robin if sticky sessions are enabled.
3. Make the authentication stateless by using JWT tokens.

### SSL/TLS Termination (HTTPS Offloading)
This is where the load balancer handles the SSL/TLS encryption and decryption (terminating HTTPS traffic) before forwarding the requests to backend servers over HTTP. This offloads the resource-intensive encryption tasks from the backend servers, improving their performance.

The first question that could raise for you is, Now my ec2 instances are acception the request over http. Is this secured?

We can make it secured by

- Use a Private Subnet: Ensure that the EC2 instances are in a private subnet within a Virtual Private Cloud (VPC) to isolate and protect unencrypted traffic.
- Restrict Access: Use security groups and network access control lists (NACLs) to restrict access to the EC2 instances, ensuring that only the ALB can communicate with them.

There could be scenarios where we need encryption between the ALB and EC2 instance. In this setup, the ALB performs SSL/TLS termination for incoming HTTPS traffic, decrypts it, and then re-encrypts the traffic before forwarding it to the EC2 instances over HTTPS. This ensures that data remains encrypted throughout its journey from the client to the EC2 instance. This is particularly useful when you are configuring an Application Load Balancer (ALB) to route traffic to both EC2 instances and on-premise servers. This approach ensures that data remains encrypted across the entire path, from the client through the ALB and finally to the destination servers, whether they are in the cloud (EC2) or on-premise.
...
![distributed-cache](./images/chapter-I/caching/distributed-cache.png)
...
![global-cache](./images/chapter-I/caching/global-cache.png)
...
![cdn-map](./images/chapter-I/content-delivery-network/cdn-map.png)
...
![cdn](./images/chapter-I/content-delivery-network/cdn.png)
...
![forward-proxy](./images/chapter-I/proxy/forward-proxy.png)
...
![reverse-proxy](./images/chapter-I/proxy/reverse-proxy.png)
...
![scalability](./images/chapter-I/scalability/scalability.png)
...
![database-types](./images/chapter-II/databases-and-dbms/database-types.png)

Below are different types of databases:

- **[SQL](./images/sql-databases)**
- **[NoSQL](./images/nosql-databases)**
  - Document
  - Key-value
  - Graph
  - Timeseries
...
SQL and NoSQL databases are broad topics and will be discussed separately in [SQL databases](./images/sql-databases) and [NoSQL databases](./images/nosql-databases). Learn how they compare to each other in [SQL vs NoSQL databases](./images/sql-vs-nosql-databases).
...
![master-slave-replication](./images/chapter-II/database-replication/master-slave-replication.png)
...
![master-master-replication](./images/chapter-II/database-replication/master-master-replication.png)
...
![indexes](./images/chapter-II/indexes/indexes.png)
...
![dense-index](./images/chapter-II/indexes/dense-index.png)
...
![sparse-index](./images/chapter-II/indexes/sparse-index.png)
...
![cap-theorem](./images/chapter-II/cap-theorem/cap-theorem.png)
...
![pacelc-theorem](./images/chapter-II/pacelc-theorem/pacelc-theorem.png)
...
![transaction-states](./images/chapter-II/transactions/transaction-states.png)
...
![two-phase-commit](./images/chapter-II/distributed-transactions/two-phase-commit.png)
...
![three-phase-commit](./images/chapter-II/distributed-transactions/three-phase-commit.png)
...
![sagas](./images/chapter-II/distributed-transactions/sagas.png)
...
![sharding](./images/chapter-II/sharding/sharding.png)
...
![simple-hashing](./images/chapter-II/consistent-hashing/simple-hashing.png)
...
![consistent-hashing](./images/chapter-II/consistent-hashing/consistent-hashing.png)
...
![virtual-nodes](./images/chapter-II/consistent-hashing/virtual-nodes.png)
...
![database-federation](./images/chapter-II/database-federation/database-federation.png)
...
![n-tier-architecture](./images/chapter-III/n-tier-architecture/n-tier-architecture.png)
...
![message-broker](./images/chapter-III/message-brokers/message-broker.png)
...
- **[Point-to-Point messaging](./images/message-queues)**: This is the distribution pattern utilized in message queues with a one-to-one relationship between the message's sender and receiver.
- **[Publish-Subscribe messaging](./images/publish-subscribe)**: In this message distribution pattern, often referred to as _"pub/sub"_, the producer of each message publishes it to a topic, and multiple message consumers subscribe to topics from which they want to receive messages.
...
[Enterprise Service Bus (ESB)](./images/enterprise-service-bus) infrastructure is complex and can be challenging to integrate and expensive to maintain. It's difficult to troubleshoot them when problems occur in production environments, they're not easy to scale, and updating is tedious.

Whereas message brokers are a _"lightweight"_ alternative to ESBs that provide similar functionality, a mechanism for inter-service communication, at a lower cost. They're well-suited for use in the [microservices architectures](./images/monoliths-microservices#microservices) that have become more prevalent as ESBs have fallen out of favor.
...
![message-queue](./images/chapter-III/message-queues/message-queue.png)
...
![publish-subscribe](./images/chapter-III/publish-subscribe/publish-subscribe.png)
...
![enterprise-service-bus](./images/chapter-III/enterprise-service-bus/enterprise-service-bus.png)
...
![monolith](./images/chapter-III/monoliths-microservices/monolith.png)
...
![microservices](./images/chapter-III/monoliths-microservices/microservices.png)
...
- Fail fast by using a [circuit breaker](./images/circuit-breaker) to achieve fault tolerance.
...
- Lack of idempotency.
- Trying to do everything [ACID instead of BASE](./images/acid-and-base-consistency-models).
...
![architecture-range](./images/chapter-III/monoliths-microservices/architecture-range.png)
...
![event-driven-architecture](./images/chapter-III/event-driven-architecture/event-driven-architecture.png)
...
- [Sagas](./images/distributed-transactions#sagas)
- [Publish-Subscribe](./images/publish-subscribe)
- [Event Sourcing](./images/event-sourcing)
- [Command and Query Responsibility Segregation (CQRS)](./images/command-and-query-responsibility-segregation)
...
![event-sourcing](./images/chapter-III/event-sourcing/event-sourcing.png)
...
Event sourcing is seemingly constantly being confused with [Event-driven Architecture (EDA)](./images/event-driven-architecture). Event-driven architecture is about using events to communicate between service boundaries. Generally, leveraging a message broker to publish and consume events asynchronously within other boundaries.
...
![command-and-query-responsibility-segregation](./images/chapter-III/command-and-query-responsibility-segregation/command-and-query-responsibility-segregation.png)
...
![api-gateway](./images/chapter-III/api-gateway/api-gateway.png)
...
- [Service discovery](./images/service-discovery)
- [Reverse Proxy](./images/proxy#reverse-proxy)
- [Caching](./images/caching)
- Security
- Retry and [Circuit breaking](./images/circuit-breaker)
- [Load balancing](./images/load-balancing)
- Logging, Tracing
- API composition
- [Rate limiting](./images/rate-limiting) and throttling
- Versioning
- Routing
...
![backend-for-frontend](./images/chapter-III/api-gateway/backend-for-frontend.png)
...
_[GraphQL](./images/rest-graphql-grpc#graphql) performs really well as a backend for frontend (BFF)._
...
![long-polling](./images/chapter-III/long-polling-websockets-server-sent-events/long-polling.png)
...
![websockets](./images/chapter-III/long-polling-websockets-server-sent-events/websockets.png)
...
![server-sent-events](./images/chapter-III/long-polling-websockets-server-sent-events/server-sent-events.png)
...
![geohashing](./images/chapter-IV/geohashing-and-quadtrees/geohashing.png)
...
![quadtree](./images/chapter-IV/geohashing-and-quadtrees/quadtree.png)
...
![quadtree-subdivision](./images/chapter-IV/geohashing-and-quadtrees/quadtree-subdivision.png)
...
![circuit-breaker](./images/chapter-IV/circuit-breaker/circuit-breaker.png)
...
![rate-limiting](./images/chapter-IV/rate-limiting/rate-limiting.png)
...
![client-side-service-discovery](./images/chapter-IV/service-discovery/client-side-service-discovery.png)
...
![server-side-service-discovery](./images/chapter-IV/service-discovery/server-side-service-discovery.png)
...
![disaster-recovery](./images/chapter-IV/disaster-recovery/disaster-recovery.png)
...
![virtualization-vs-containerization](./images/chapter-IV/virtual-machines-and-containers/virtualization-vs-containerization.png)
...
![oauth2](./images/chapter-IV/oauth2-and-openid-connect/oauth2.png)
...
![sso](./images/chapter-IV/single-sign-on/sso.png)
...
**Long URL**: [./images/url-shortener](./images/url-shortener)
...
![url-shortener-datamodel](./images/chapter-V/url-shortener/url-shortener-datamodel.png)
...
Contains the new short URL's properties such as `expiration`, `hash`, `originalURL`, and `userID` of the user who created the short URL. We can also use the `hash` column as an [index](./images/indexes) to improve the query performance.
...
_For more details, refer to [SQL vs NoSQL](./images/sql-vs-nosql-databases)._
...
Now, let's talk about [caching](./images/caching). As per our estimations, we will require around ~35 GB of memory per day to cache 20% of the incoming requests to our services. For this use case, we can use [Redis](https://redis.io) or [Memcached](https://memcached.org) servers alongside our API server.

_For more details, refer to [caching](./images/caching)._

### Design

Now that we have identified some core components, let's do the first draft of our system design.

![url-shortener-basic-design](./images/chapter-V/url-shortener/url-shortener-basic-design.png)
...
To scale out our databases we will need to partition our data. Horizontal partitioning (aka [Sharding](./images/sharding)) can be a good first step. We can use partitions schemes such as:
...
The above approaches can still cause uneven data and load distribution, we can solve this using [Consistent hashing](./images/consistent-hashing).

_For more details, refer to [Sharding](./images/sharding) and [Consistent Hashing](./images/consistent-hashing)._
...
Now let us talk about [caching](./images/caching).
...
We can also use an [API Gateway](./images/api-gateway) as they can support capabilities like authorization, rate limiting, and load balancing out of the box.

## Identify and resolve bottlenecks

![url-shortener-advanced-design](./images/chapter-V/url-shortener/url-shortener-advanced-design.png)
...
- Introducing [load balancers](./images/load-balancing) between clients, servers, databases, and cache servers.
...
![whatsapp-datamodel](./images/chapter-V/whatsapp/whatsapp-datamodel.png)
...
We will be using [microservices architecture](./images/monoliths-microservices#microservices) since it will make it easier to horizontally scale and decouple our services. Each service will have ownership of its own data model. Let's try to divide our system into some core services.
...
Since our architecture is microservices-based, services will be communicating with each other as well. Generally, REST or HTTP performs well but we can further improve the performance using [gRPC](./images/rest-graphql-grpc#grpc) which is more lightweight and efficient.

[Service discovery](./images/service-discovery) is another thing we will have to take into account. We can also use a service mesh that enables managed, observable, and secure communication between individual services.

_Note: Learn more about [REST, GraphQL, gRPC](./images/rest-graphql-grpc) and how they compare with each other._
...
The client can periodically send an HTTP request to servers to check if there are any new messages. This can be achieved via something like [Long polling](./images/long-polling-websockets-server-sent-events#long-polling).

**Push model**

The client opens a long-lived connection with the server and once new data is available it will be pushed to the client. We can use [WebSockets](./images/long-polling-websockets-server-sent-events#websockets) or [Server-Sent Events (SSE)](./images/long-polling-websockets-server-sent-events#server-sent-events-sse) for this.

The pull model approach is not scalable as it will create unnecessary request overhead on our servers and most of the time the response will be empty, thus wasting our resources. To minimize latency, using the push model with [WebSockets](./images/long-polling-websockets-server-sent-events#websockets) is a better choice because then we can push data to the client once it's available without any delay, given that the connection is open with the client. Also, WebSockets provide full-duplex communication, unlike [Server-Sent Events (SSE)](./images/long-polling-websockets-server-sent-events#server-sent-events-sse) which are only unidirectional.

_Note: Learn more about [Long polling, WebSockets, Server-Sent Events (SSE)](./images/long-polling-websockets-server-sent-events)._
...
If the recipient is not active, the chat service will add an event to a [message queue](./images/message-queues) with additional metadata such as the client's device platform which will be used to route the notification to the correct platform later on.
...
![whatsapp-basic-design](./images/chapter-V/whatsapp/whatsapp-basic-design.png)
...
But where can we store files at scale? Well, [object storage](./images/storage#object-storage) is what we're looking for. Object stores break data files up into pieces called objects. It then stores those objects in a single repository, which can be spread out across multiple networked systems. We can also use distributed file storage such as [HDFS](./images/storage#hdfs) or [GlusterFS](https://www.gluster.org).
...
[Content Delivery Network (CDN)](./images/content-delivery-network) increases content availability and redundancy while reducing bandwidth costs. Generally, static files such as images, and videos are served from CDN. We can use services like [Amazon CloudFront](https://aws.amazon.com/cloudfront) or [Cloudflare CDN](https://www.cloudflare.com/cdn) for this use case.

### API gateway

Since we will be using multiple protocols like HTTP, WebSocket, TCP/IP, deploying multiple L4 (transport layer) or L7 (application layer) type load balancers separately for each protocol will be expensive. Instead, we can use an [API Gateway](./images/api-gateway) that supports multiple protocols without any issues.
...
![whatsapp-advanced-design](./images/chapter-V/whatsapp/whatsapp-advanced-design.png)
...
- Exactly once delivery and message ordering is challenging in a distributed system, we can use a dedicated [message broker](./images/message-brokers) such as [Apache Kafka](https://kafka.apache.org) or [NATS](https://nats.io) to make our notification system more robust.
...
![twitter-datamodel](./images/chapter-V/twitter/twitter-datamodel.png)
...
![newsfeed-pull-model](./images/chapter-V/twitter/newsfeed-pull-model.png)
...
![newsfeed-push-model](./images/chapter-V/twitter/newsfeed-push-model.png)
...
_For more details, refer to the [WhatsApp](./images/whatsapp#notifications) system design where we discuss push notifications in detail._
...
![twitter-advanced-design](./images/chapter-V/twitter/twitter-advanced-design.png)
...
![netflix-datamodel](./images/chapter-V/netflix/netflix-datamodel.png)
...
![video-processing-pipeline](./images/chapter-V/netflix/video-processing-pipeline.png)
...
Here's how we can process videos once they're uploaded by the content team (or users in YouTube's case) and are queued for processing in our [message queue](./images/message-queues).

Let's discuss how this works:

- **File Chunker**

![file-chunking](./images/chapter-V/netflix/file-chunking.png)
...
This entire process is done by a machine learning model which performs copyright, piracy, and NSFW checks. If issues are found, we can push the task to a [dead-letter queue (DLQ)](./images/message-queues#dead-letter-queues) and someone from the moderation team can do further inspection.
...
It allows us to fetch the desired quality of the video as per the user's request, and once the media file finishes processing, it gets uploaded to a distributed file storage such as [HDFS](./images/storage#hdfs), [GlusterFS](https://www.gluster.org), or an [object storage](./images/storage#object-storage) such as [Amazon S3](https://aws.amazon.com/s3) for later retrieval during streaming.

_Note: We can add additional steps such as subtitles and thumbnails generation as part of our pipeline._

**Why are we using a message queue?**

Processing videos as a long-running task and using a [message queue](./images/message-queues) makes much more sense. It also decouples our video processing pipeline from the upload functionality. We can use something like [Amazon SQS](https://aws.amazon.com/sqs) or [RabbitMQ](https://www.rabbitmq.com) to support this.

### Video streaming

Video streaming is a challenging task from both the client and server perspectives. Moreover, internet connection speeds vary quite a lot between different users. To make sure users don't re-fetch the same content, we can use a [Content Delivery Network (CDN)](./images/content-delivery-network).
...
Netflix Open Connect is a purpose-built [Content Delivery Network (CDN)](./images/content-delivery-network) responsible for serving Netflix's video traffic. Around 95% of the traffic globally is delivered via direct connections between Open Connect and the ISPs their customers use to access the internet.
...
_For more details, refer to the [URL Shortener](./images/url-shortener) system design._
...
We will use distributed file storage such as [HDFS](./images/storage#hdfs), [GlusterFS](https://www.gluster.org), or an [object storage](./images/storage#object-storage) such as [Amazon S3](https://aws.amazon.com/s3) for storage and streaming of the content.
...
![netflix-advanced-design](./images/chapter-V/netflix/netflix-advanced-design.png)
...
![uber-datamodel](./images/chapter-V/uber/uber-datamodel.png)
...
![uber-working](./images/chapter-V/uber/uber-working.png)
...
The client can periodically send an HTTP request to servers to report its current location and receive ETA and pricing information. This can be achieved via something like [Long polling](./images/long-polling-websockets-server-sent-events#long-polling).
...
[Geohashing](./images/geohashing-and-quadtrees#geohashing) is a [geocoding](https://en.wikipedia.org/wiki/Address_geocoding) method used to encode geographic coordinates such as latitude and longitude into short alphanumeric strings. It was created by [Gustavo Niemeyer](https://twitter.com/gniemeyer) in 2008.
...
A [Quadtree](./images/geohashing-and-quadtrees#quadtrees) is a tree data structure in which each internal node has exactly four children. They are often used to partition a two-dimensional space by recursively subdividing it into four quadrants or regions. Each child or leaf node stores spatial information. Quadtrees are the two-dimensional analog of [Octrees](https://en.wikipedia.org/wiki/Octree) which are used to partition three-dimensional space.
...
_For more details, refer to [Transactions](./images/transactions) and [Distributed Transactions](./images/distributed-transactions)._
...
![uber-advanced-design](./images/chapter-V/uber/uber-advanced-design.png)
